...
try:
    if getGlobalProperty('running'):
        try:
            xbmc.executebuiltin('NotifyAll({0},{1},{2})'.format('script.royfelott', 'RESTORE', '{}'))
        except:
            log('Main: script.royfelott: Already running, couldn\'t reactivate other instance, exiting.')
    else:
        if not getGlobalProperty('started'):
            if getGlobalProperty('waiting_for_start'):
                setGlobalProperty('waiting_for_start', '')
                log('Main: script.royfelott: Currently waiting for start, immediate start was requested.')
                sys.exit(0)

            with SingleInstance("royfel"):
                started = True
                ...
                if boot_delay:
                    ...
                    log('Main: script.royfelott: Delaying start for {}s.', boot_delay)
                ...
                if waited < boot_delay:
                    log('Main: script.royfelott: Forced start before auto-start delay ({:.1f}/{} s).', waited, boot_delay)
                setGlobalProperty('waiting_for_start', '')
                setGlobalProperty('started', '1')

                main.main()
        else:
            log('Main: script.royfelott: Already running, exiting')
